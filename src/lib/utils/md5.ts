/**
 * 纯 JS MD5 实现（RFC 1321），用于非安全场景的字符串哈希
 * 输出与 Python hashlib.md5(s.encode('utf-8')).hexdigest() 一致
 */
...
function binl2hex(binarray: number[]) {
  ...
  for (let i = 0; i < binarray.length * 32; i += 8)
    str += hexTab.charAt((binarray[i >> 5] >>> (i % 32 + 4)) & 0xf) + hexTab.charAt((binarray[i >> 5] >>> (i % 32)) & 0xf)
  return str
}

/** 计算字符串的 MD5 哈希（hex），与 Python hashlib.md5(s.encode('utf-8')).hexdigest() 一致 */
export function md5(str: string): string {
  const { bin, len } = str2binl(str)
  return binl2hex(binlMD5(bin, len))
}
