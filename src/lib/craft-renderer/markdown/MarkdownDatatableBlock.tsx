...
// ── Inline markup parser（**粗体** / ***风险*** / <mark>标签</mark>）────────
...
function renderInlineMarkup(text: string): React.ReactNode {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('**') && part.endsWith('**')) {
      return <strong key={i} className="text-primary/90" style={{ fontWeight: 600 }}>{part.slice(2, -2)}</strong>
    }
...
function formatCell(value: unknown, type?: ColumnDef['type']): React.ReactNode {
  ...
  switch (type) {
    ...
    case 'badge': {
      ...
      const color =
        // 正向状态：绿色
        s === 'active' || s === 'passing' || s === 'success' || s === 'done' || s === '优秀' || s === '良好' || s === '高'
        ? 'bg-green-500/10 text-green-600'
        // 中性状态：蓝色（Semi 主色）
        : s === 'pending' || s === 'running' || s === 'info' || s === '一般' || s === '中' || s === '中等'
        ? 'bg-primary/10 text-primary'
        // 警告状态：橙色
        : s === 'warning' || s === '偏低' || s === '低' || s === '注意'
        ? 'bg-orange-500/10 text-orange-600'
        // 错误状态：红色
        : s === 'revoked' || s === 'failed' || s === 'error' || s === '差' || s === '不适合' || s === '极低'
    ...
    default:
      return renderInlineMarkup(String(value))
  }
}
      ...
      <DropdownMenuTrigger asChild>
        <button
          className={cn(
            ...
            hasActiveControls
              ? 'opacity-100 bg-primary/5 text-primary shadow-tinted'
              : alwaysVisible
      ...
      <StyledDropdownMenuContent sideOffset={6} align="end" className="min-w-36" style={{ zIndex: 400 }}>
        ...
        <DropdownMenuSub>
          <StyledDropdownMenuSubTrigger>
            ...
            {sortKey && sortDir && <Check className="w-3 h-3 text-primary" />}
          </StyledDropdownMenuSubTrigger>
        ...
        <DropdownMenuSub>
          <StyledDropdownMenuSubTrigger>
            ...
            {groupKey && <Check className="w-3 h-3 text-primary" />}
          </StyledDropdownMenuSubTrigger>
          <StyledDropdownMenuSubContent style={{ zIndex: 401 }}>
            {parsed.columns.map((col) => {
                    ...
                    <StyledDropdownMenuSubContent style={{ zIndex: 402 }}>
                      {opts.map((opt) => {
                        ...
                        return (
                          ...
                          >
                            <span className={cn('flex-1', optActive && 'text-primary font-medium')}>{opt.label}</span>
                            {optActive && <Check className="w-3.5 h-3.5 text-primary" />}
                          </StyledDropdownMenuItem>
                        )
                      })}
                    </StyledDropdownMenuSubContent>
