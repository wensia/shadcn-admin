 ...
 * - Left-click → Base UI Menu with "Open" + "Reveal in Finder"
 * - Right-click → Base UI ContextMenu with the same items
 *
...
// ============================================================================
// Shared menu item styling — used by both Menu and ContextMenu
// ============================================================================
 ...
 *
 * Implementation: Wraps a Base UI Menu (left-click trigger) inside a
 * Base UI ContextMenu (right-click trigger). Both render the same menu items.
...
function FilePathBadge({ filePath }: FilePathBadgeProps) {
  ...
  // Menu items rendered inside Menu.Popup
  const dropdownItems = (
    <>
      {onOpenFileExternal && (
        <Menu.Item className={menuItemClasses} onClick={handleOpen}>
          <ExternalLink className="w-3.5 h-3.5 shrink-0" />
          Open
        </Menu.Item>
      )}
      {onRevealInFinder && (
        <Menu.Item className={menuItemClasses} onClick={handleReveal}>
  ...
  const contextItems = (
    <>
      {onOpenFileExternal && (
        <ContextMenu.Item className={menuItemClasses} onClick={handleOpen}>
          <ExternalLink className="w-3.5 h-3.5 shrink-0" />
          Open
        </ContextMenu.Item>
      )}
      {onRevealInFinder && (
        <ContextMenu.Item className={menuItemClasses} onClick={handleReveal}>
          <FolderOpen className="w-3.5 h-3.5 shrink-0" />
          Reveal in Finder
        </ContextMenu.Item>
      )}
  ...
  // Wrap: ContextMenu (right-click) wraps Menu (left-click) wraps the badge
  return (
    <ContextMenu.Root>
      <ContextMenu.Trigger
        render={
          <Menu.Root>
            ...
            <Menu.Portal>
              <Menu.Positioner sideOffset={6} align="center">
                <Menu.Popup className={menuContentClasses}>
                  {dropdownItems}
                </Menu.Popup>
              </Menu.Positioner>
            </Menu.Portal>
      ...
      <ContextMenu.Portal>
        <ContextMenu.Positioner>
          <ContextMenu.Popup className={menuContentClasses}>
            {contextItems}
          </ContextMenu.Popup>
        </ContextMenu.Positioner>
      </ContextMenu.Portal>
