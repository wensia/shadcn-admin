...
import { z } from 'zod'
import { Briefcase, Plus, Pencil, Trash2, RefreshCw, Search } from 'lucide-react'
...
const formSchema = z.object({
  name: z.string().min(1, '请输入职位名称').max(50, '名称最多50个字符'),
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          return <PositionLevelBadge level={row.original.level} />
        },
      },
      {
      ...
      <div className="flex flex-col gap-4 h-full">
        ...
        <div className="flex items-center justify-between flex-shrink-0">
          <div>
            ...
            <p className="text-muted-foreground text-sm">管理系统中的职位信息</p>
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] p-0 flex flex-col">
          <DialogHeader className="px-6 pt-6 pb-4 shrink-0">
            <DialogTitle>{editingItem ? '编辑职位' : '新建职位'}</DialogTitle>
            <DialogDescription>
              {editingItem ? '修改职位信息' : '创建一个新的职位'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      <FormControl>
                        <SelectTrigger>
                          <SelectValue placeholder="请选择职位级别" />
                        </SelectTrigger>
                      </FormControl>
                      <SelectContent>
                        {POSITION_LEVELS.map((level) => (
