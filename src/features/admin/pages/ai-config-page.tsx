...
import { Settings2, FileText, Layers, BookOpen, Activity, FlaskConical } from 'lucide-react'
...
import { AIPromptTestLab } from '../components/integrations/ai-prompt-test-lab'
import { AIDocumentLibrary } from '../components/integrations/ai-document-library'
import { AIUsageLogContent } from '../components/integrations/ai-usage-log-content'
...
export function AIConfigPage() {
  ...
  return (
    <Tabs defaultActiveKey="models" className="flex h-full flex-col overflow-hidden pt-2 px-1">
      ...
      <TabPane tab={<span style={{display:'inline-flex',alignItems:'center',gap:8}}><FlaskConical size={16}/>Prompt 测试</span>} itemKey="prompt-test">
        <AIPromptTestLab />
      </TabPane>
      <TabPane tab={<span style={{display:'inline-flex',alignItems:'center',gap:8}}><BookOpen size={16}/>资料库</span>} itemKey="documents">
        <AIDocumentLibrary />
      </TabPane>
      <TabPane tab={<span style={{display:'inline-flex',alignItems:'center',gap:8}}><Activity size={16}/>调用记录</span>} itemKey="usage-logs">
        <AIUsageLogContent />
      </TabPane>
    </Tabs>
  )
