...
const formSchema = z.object({
  name: z.string().min(1, '请输入大区名称').max(50, '名称最多50个字符'),
...
function createSkeletonData(count: number): RegionItem[] {
...
export function RegionsPage() {
  ...
  // 列定义
  const columns: ColumnDef<RegionItem>[] = useMemo(
    () => [
      {
        accessorKey: 'name',
        header: '大区名称',
    ...
    <Main fixed>
      <div className="flex flex-col gap-4 h-full">
        ...
        <div className="flex items-center justify-between flex-shrink-0">
          <div>
            <h1 className="text-2xl font-semibold">大区管理</h1>
            <p className="text-muted-foreground text-sm">管理系统中的大区信息</p>
          </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] p-0 flex flex-col">
          <DialogHeader className="px-6 pt-6 pb-4 shrink-0">
            <DialogTitle>{editingItem ? '编辑大区' : '新建大区'}</DialogTitle>
            <DialogDescription>
              {editingItem ? '修改大区信息' : '创建一个新的大区'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSubmit)} className="flex flex-col flex-1 min-h-0">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>大区名称 *</FormLabel>
                    <FormControl>
                      <Input placeholder="请输入大区名称" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
